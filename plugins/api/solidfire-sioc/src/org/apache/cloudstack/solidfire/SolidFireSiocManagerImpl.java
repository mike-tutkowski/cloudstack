...
public class SolidFireSiocManagerImpl implements SolidFireSiocManager {
    private static final Logger LOGGER = Logger.getLogger(SolidFireSiocManagerImpl.class);
    private static final int LOCK_TIME_IN_SECONDS = 3;
    private static final int ONE_GB_IN_BYTES = 10 ^ 9;
    private static final int LOWEST_SHARES_PER_VIRTUAL_DISK = 200; // VMware limit
    private static final int HIGHEST_SHARES_PER_VIRTUAL_DISK = 4000; // VMware limit
    private static final long DEFAULT_LIMIT_IOPS_PER_VIRTUAL_DISK = 1000;
    ...
    @Override
    public void updateSiocInfo(long zoneId, String storageTag, Optional<Integer> sharesPerGB, Optional<Integer> limitIopsPerGB) throws Exception {
        LOGGER.info("'SolidFireSiocManagerImpl.updateSiocInfo(long, String)' method invoked");

        if (sharesPerGB.isPresent() && !limitIopsPerGB.isPresent()) {
            throw new Exception("If 'sharespergb' is passed in, 'limitiopspergb' needs to be passed in.");
        }

        if (limitIopsPerGB.isPresent() && !sharesPerGB.isPresent()) {
            throw new Exception("If 'limitiopspergb' is passed in, 'sharespergb' needs to be passed in.");
        }
            ...
            for (StoragePoolDetailVO detail : details) {
                if (storageTag.equalsIgnoreCase(detail.getName()) && Boolean.TRUE.toString().equalsIgnoreCase(detail.getValue())) {
                    ...
                    if (storagePool != null && storagePool.getDataCenterId() == zoneId &&
                            storagePool.getPoolType().equals(StoragePoolType.VMFS)) {
                        ...
                        if (volumes != null && volumes.size() > 0) {
                            ...
                            for (Long instanceId : instanceIds) {
                                List<ManagedObjectReference> tasks = updateSiocInfo(instanceId, storagePool, connection, sharesPerGB, limitIopsPerGB);
    ...
    private List<ManagedObjectReference> updateSiocInfo(Long instanceId, StoragePoolVO storagePool,
            VMwareUtil.VMwareConnection connection, Optional<Integer> sharesPerGB, Optional<Integer> limitIopsPerGB) throws Exception {
        List<ManagedObjectReference> tasks = new ArrayList<>();

        VMInstanceVO vmInstance = _vmInstanceDao.findById(instanceId);

        if (vmInstance == null) {
            LOGGER.warn(SolidFireSiocManagerImpl.class.getName() + ": " + "The VM with ID " + instanceId + " could not be located.");
        ...
        for (VirtualDevice device : devices) {
            if (device instanceof VirtualDisk) {
                ...
                if (volumeVO != null) {
                    ...
                    int newShares = getNewShares(volumeVO, sharesPerGB);
                    ...
                    long newLimitIops = getNewLimitIops(volumeVO, limitIopsPerGB);
                    ...
                    if (diskUpdated) {
                        ...
                        try {
                            ...
                            LOGGER.warn(SolidFireSiocManagerImpl.class.getName() + ": " + ex.getMessage());
                        }
                    }
                }
            }
    ...
    private String getInfoMsg(Volume volume, Integer newShares, Long newLimitIops) {
        String msgPrefix = "SolidFire SIOC: Volume = " + volume.getName();
    ...
    private int getNewShares(VolumeVO volumeVO, Optional<Integer> sharesPerGB) {
        if (sharesPerGB.isPresent()) {
            return getNewSharesBasedOnVolumeSize(volumeVO, sharesPerGB.get());
        }

        return getNewShares(volumeVO);
    }
    ...
    private int getNewSharesBasedOnVolumeSize(VolumeVO volumeVO, int sharesPerGB) {
        ...
        int shares = (int)(sharesPerGB * sizeInGB);

        return getAdjustedShares(shares);
    }

    private int getNewShares(VolumeVO volumeVO) {
        if (volumeVO.getMinIops() != null) {
            // it's OK to convert from Long to int here (the Min IOPS top out at 15,000 per volume)
            return getAdjustedShares(volumeVO.getMinIops().intValue());
        }

        return LOWEST_SHARES_PER_VIRTUAL_DISK;
    }
    ...
    private long getNewLimitIops(VolumeVO volumeVO, Optional<Integer> limitIopsPerGB) {
        if (limitIopsPerGB.isPresent()) {
            return getNewLimitIopsBasedOnVolumeSize(volumeVO, limitIopsPerGB.get());
        }

        return getNewLimitIops(volumeVO);
    }
    ...
    private long getNewLimitIopsBasedOnVolumeSize(VolumeVO volumeVO, int limitIopsPerGB) {
        ...
        return (long)(limitIopsPerGB * sizeInGB);
    }

    private long getNewLimitIops(VolumeVO volumeVO) {
        if (volumeVO.getMaxIops() != null) {
            return volumeVO.getMaxIops();
        }

        return DEFAULT_LIMIT_IOPS_PER_VIRTUAL_DISK;
    }
